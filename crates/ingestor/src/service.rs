...
use crate::{GrabbedImage, GrabbedPost, IngestorConfig, SourceAdapter};

pub struct ImageGrabberService {
    adapters: Vec<Box<dyn SourceAdapter>>,
}

impl ImageGrabberService {
    pub fn new(adapters: Vec<Box<dyn SourceAdapter>>) -> Self {
        Self { adapters }
    }

    pub async fn grab_all(&self) -> Result<Vec<GrabbedPost>> {
...
pub async fn run_to_directory(config: &IngestorConfig, output_dir: &Path) -> Result<usize> {
    ...
    let service = ImageGrabberService::new(adapters);
    let posts = service.grab_all().await?;
